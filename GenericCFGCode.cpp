...
public:
    ...
    Production* currentProduction = startcfg;
    while (currentProduction != nullptr) {
        Rule* currentRule = currentProduction->rule_list;
        Rule* prevRule = nullptr;
        while (currentRule != nullptr) {
            // Check if the rule is a null production
            if (currentRule->rule == "^") {
                // Remove the null production
                if (prevRule != nullptr) {
                    prevRule->next = currentRule->next;
                } else {
                    currentProduction->rule_list = currentRule->next;
                }
                Rule* tempRule = currentRule;
                currentRule = currentRule->next;
                delete tempRule;
                continue; // Move to the next rule after deletion
            }
